...
bot = telebot.TeleBot('7515189541:AAFSBgoS3GXKIjk0X6WEb9k8pTP66nM-gfw')
...
def send_welcome(message):
    response = (
        f"🌟 Welcome to the DARINDA OFFICIAL DDOS Bot! 🌟\n\n"
...
def approve_user(message):
    ...
    if user_id in admin_ids or user_id == owner_id:
        ...
        response = "Only @its_darinda Can Run This Command 😡."
    bot.send_message(message.chat.id, response)
...
def add_admin(message):
    ...
    if user_id == owner_id:
        ...
        response = "Only the @its_darinda Can Run This Command 😡."
    bot.send_message(message.chat.id, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /attack command. "
                    ...
                    "🌟THE ONLY OWNER IS @its_darinda DM TO BUY ACCESS")
...
def handle_unknown_command(message):
    response = (
        f"🌟 Welcome to the @its_darinda OFFICIAL DDOS Bot! 🌟\n\n"
        f"Current Time: {get_current_time()}\n\n"
        "Here are some commands you can use:\n"
        "❌ /removeuser <id> - Remove a user\n"
        "🔑 /addadmin <id> <balance> - Add an admin with a starting balance\n"
